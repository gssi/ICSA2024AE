...
public class RA2HTML extends EpsilonLiveFunction {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(new RA2HTML().run(raFlexmi));
	...
	public void serviceImpl(JsonObject request, JsonObject response) throws Exception {
		...
		if (request.get("mode") == null) {
			raFlexmi = Files.readString(
					Paths.get(new File("src/main/resources/models/" + request.get("ra").getAsString()).toURI()));
			html = run(raFlexmi);
		} else {
		...
		if (validationresults != null && validationresults.size() > 0)
			response.addProperty("errors", validationresults.toString() + "\n");
		response.addProperty("output", (exectime + "\n") + bos.toString());
	}

	public Collection<UnsatisfiedConstraint> runRAValidation(String RAFlexmi, String evl, OutputStream outputStream,
			JsonObject response) {
		...
		try {
			...
			String raEmfatic = Files
					.readString(Paths.get(new File("src/main/resources//metamodels/moremm.emf").toURI()));

			module.getContext().setOutputStream(new PrintStream(outputStream));

			// return runEvl((EvlModule) module, kpiFlexmi, kpiEmfatic, scFlexmi, scEmfatic,
			// response);
